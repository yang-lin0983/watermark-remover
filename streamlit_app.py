    ...
    if mode == "自动检测":
        if st.button("🚀 开始自动去除水印", type="primary"):
            with st.spinner("正在检测水印区域..."):
                try:
                    result = remover.remove_watermark(image, mode='auto')
        ...
        st.info("💡 提示：在下方图片上点击并拖动来选择水印区域")
        
        # 简化的手动选择（使用相对坐标）
        st.markdown("请输入水印区域坐标（相对于图片的百分比）：")
        ...
        st.image(preview, channels="BGR", caption="预览选择区域", use_container_width=True)
        
        if st.button("🚀 去除选定区域水印", type="primary"):
            with st.spinner("正在处理..."):
                try:
                    regions = [(x_px, y_px, w_px, h_px)]
                    result = remover.remove_watermark(image, mode='manual', regions=regions)
